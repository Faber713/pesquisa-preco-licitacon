...
from config import MAX_RESULTADOS, LINK_BASE_LICITACON
from util import preparar_js
...
""")
        ...
        for i, r in enumerate(resultados[:MAX_RESULTADOS], start=1):
            ...
            f.write(f'<div class="{score_class}">Resultado {i} - {r["score"]}%</div>')
            ...
            f.write(f"<p><b>Valor unitário:</b> {escape(str(r.get('valor_unitario', '')))}</p>")
            ...
            f.write(f"Valor unitário: {escape(str(r.get('valor_unitario','')))}<br>")

            copiar_tudo = preparar_js(
                f"Órgão: {r['orgao']} | Descrição: {descricao_link} | Quantidade: {r.get('qtd','')} | Valor: {r.get('valor_unitario','')}"
            )

            f.write(f"<button class='copy-all-btn' onclick=\"copiarTexto('{copiar_tudo}')\">Copiar tudo</button>")
            f.write('</div>')

            f.write(f'<a class="botao" href="{LINK_BASE_LICITACON}" target="_blank">Abrir LicitaCon</a>')
