...
- Preservar os pontos de entrada atuais, especialmente `run.py`, `web/app.py`, `search/busca.py` e importadores.
...
- `storage/app_storage.py`: persistencia da aplicacao.
- `web/`: UI Flask com templates Jinja, assets estaticos, rotas HTML e APIs JSON.

O principal risco arquitetural hoje e manter regras de negocio dentro de rotas. Isso dificulta testes, performance, troca futura de Flask/FastAPI e evolucao da busca.
...
Mesmo com Flask, o backend deve oferecer funcoes estaveis:
...
- Mover logica pura das rotas para servicos novos, mantendo contratos estaveis.
...
- Separar frontend HTML/JS da camada de aplicacao.
...
O primeiro refactor seguro e manter a busca SQLite em modulo dedicado, com rotas Flask finas delegando para services. Isso reduz acoplamento e preserva FTS5 sem mexer no motor.
